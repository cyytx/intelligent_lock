...
Stat = STA_NOINIT;
  ...
  printf("SD_initialize success,status:%d\r\n",Stat);
  return Stat;
}
...
{
    uint8_t ret = SD_ReadBlocks_DMA(buff, (uint64_t)(sector), count);
    return ret;  
}
...
{
    uint8_t ret = SD_WriteBlocks_DMA(buff, (uint64_t)(sector), count);
    return ret;
}
